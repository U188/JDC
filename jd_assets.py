...
if __name__== '__main__':
    ...
    for m in cookies:
        ...
        for p in logh:
            ...
            if 'bean_change' in p:
                bean_chage=spiltlog.jdparagraph(txt,pin)
            if 'speed_sign' in p:
                speed_sign = spiltlog.jsparagraph(txt,pin)
            if 'jd_fruit' in p:
                nc = spiltlog.ncparagraph(txt,pin)
            if 'jd_wsdlb' in p:
                dlb = spiltlog.dlbparagraph(txt,pin)
            if 'jd_pet' in p:
                mmc = spiltlog.mmcparagraph(txt,pin)
            if 'jd_jxmc' in p:
                mc=spiltlog.mcparagraph(txt,pin)
            if 'jd_dreamFactory' in p:
                gc=spiltlog.gcparagraph(txt,pin)
